...
class StarlingAPI: NSObject {
    ...
    required init(token: Token?) {
        ...
        self.baseURL = StarlingAPI.sandboxBaseURL
        self.authBaseURL = StarlingAPI.sandboxAuthBaseURL
    }
}
...
/*
 ...
 1. Redirect the user to Starling to authorise your app
 2. Starling redirects the user back to your app with an authorization code
 3. Exchange the authorisation code for an access token
 */
extension StarlingAPI: OAuth2 {

    // 1. Redirect user to Starling website
    ...
    // 2. Callback method, from AppDelegate, as response from Starling email
    ...
    // 3. Exchange the authorisation for an access token
